...
sealed class ScalikeJDBC[F[_]](implicit I: InjectK[Query, F]) {
  ...
  def foldLeft[A](sql: SQL[_, NoExtractor])(init: A)(f: (A, WrappedResultSet) => A): Free[F, A] =
    lift(Fold(sql, init, f))
  def foldLeft[A](sql: SQLBuilder[_])(init: A)(f: (A, WrappedResultSet) => A): Free[F, A] =
    foldLeft(withSQL(sql))(init)(f)
...
object ScalikeJDBC {
  implicit def instance[F[_]](implicit I: InjectK[Query, F]): ScalikeJDBC[F] = new ScalikeJDBC[F]
}
