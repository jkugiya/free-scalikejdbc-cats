...
sealed abstract class Query[A](private[free] val statement: String, private[free] val parameters: collection.Seq[Any]) {}
...
object Query {
  ...
  case class Fold[A](sql: SQL[_, NoExtractor], init: A, f: (A, WrappedResultSet) => A)
      extends Query[A](sql.statement, sql.parameters)
